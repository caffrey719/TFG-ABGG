function [t_out, xl_out, x_out, y_out, x_stick_out, x_slip_out, xdot_stick_out, xdot_slip_out, theta_equilibrio, ...
    ...
    landa1_slip_out, t_slip_out, t_stick_out] = Simula_Horizontal(f, masa, k, R, A, theta_o, uk, us)
...
ydot_out = []; xdot_out = []; theta_out = []; thetadot_out = []; t_out = []; x_out = []; y_out = []; 
xl_out = []; vpata_out = []; x_stick_out = {}; x_slip_out = {}; xl_stick_out = {}; xl_slip_out = {}; 
xldot_stick_out = {}; xldot_slip_out = {}; y_stick_out = {}; y_slip_out = {}; ydot_stick_out = {}; 
ydot_slip_out = {}; xdot_stick_out = {}; xdot_slip_out = {}; thetadot_slip_out = {}; thetadot_stick_out = {}; 
vpata_stick_out = {}; friccion_stick_out = {}; friccion_slip_out = {}; normal_stick_out = {}; 
normal_slip_out = {}; vpata_slip_out = {}; eta_ydd_stick_out = {}; eta_ydd_slip_out = {}; t_slip_out = {}; 
t_stick_out = {}; theta_slip_out = {}; theta_stick_out = {}; fuerzasx_stick_out = {}; fuerzasx_slip_out = {};
fuerzasy_stick_out = {}; fuerzasy_slip_out = {}; landa1_stick_out = {}; landa1_slip_out = {};
fuerzayeta_stick_out = {}; fuerzayeta_slip_out = {};
...
F3 = - masa * R^2 * sin(theta) * cos(theta) * diff(theta)^2 - masa * R * cos(theta) * (eta_xdd) + masa * g * R * sin(theta) - k * (theta - theta_o);
...
eta_x1 = A * sin(Omega * t1);
eta_xdd1 = - Omega^2 * eta_x1;
...
F1_b = - masa * g * R * Mu * cos(theta1) - masa * g * R * sin(theta1) + k * (theta1 - theta_o);
F1 = - (F1_a + F1_b);
F2 = - masa * eta_xdd1 + masa * R * Mu * cos(theta1) * diff(theta1)^2 - Mu * masa * g;
...
while tstart < tend
    ...
    F3_ = - masa * R ^2 * sin(s_(:, 3)) .* cos(s_(:, 3)) .* s_(:, 4) .^ 2 - masa * R * cos(s_(:, 3)) .* (eta_xdd_) + masa * g * R * sin(s_(:, 3)) - k * (s_(:, 3) - theta_o);
    ...
    [m, n] = size(s_);
     ...
     for paso = 1:m
         ...
         fuerzas_sistema = abs(- R * segundaderivadatheta(paso) .* sin(s_(paso, 3)) - R * s_(paso, 4) .^ 2 .* cos(s_(paso, 3)) + g) * us;
    ...
    theta_out = [theta_out; s_(1:paso - 1, 3)];
    thetadot_out = [thetadot_out; s_(1:paso - 1, 4)];
    x_out = [x_out; s_(1:paso - 1, 1)];
    xdot_out = [xdot_out; s_(1:paso - 1, 2)];
    y_out = [y_out; R * cos(s_(1:paso - 1, 3))];
    ydot_out = [ydot_out; - R * s_(1:paso - 1, 4) .* sin(s_(1:paso - 1, 3))];
    vpata_out = [vpata_out; s_(1:paso - 1, 2) - R * s_(1:paso - 1, 4) .* cos(s_(1:paso - 1, 3))];
    ...
    eta_ydd_stick_out{end + 1} = 0;
    fuerzasy_stick_out{end + 1} = masa * g;
    fuerzayeta_stick_out{end + 1} = masa * eta_ydd_stick_out{end};
    normal_stick_out{end + 1} = masa * g + masa * (-R .* segundaderivadatheta(1:paso - 1) .* sin(s_(1:paso - 1, 3)) - R .* s_(1:paso - 1, 4) .^ 2 .* cos(s_(1:paso - 1, 3)));
    friccion_stick_out{end + 1} = us * (masa * g + masa * (-R .* segundaderivadatheta(1:paso - 1) .* sin(s_(1:paso - 1, 3)) - R .* s_(1:paso - 1, 4) .^ 2 .* cos(s_(1:paso - 1, 3))));
    fuerzasx_stick_out{end + 1} = (masa .* segundaderivadax(1:paso - 1)) - us * (masa * g + masa * (-R .* segundaderivadatheta(1:paso - 1) .* sin(s_(1:paso - 1, 3)) - R .* s_(1:paso - 1, 4) .^ 2 .* cos(s_(1:paso - 1, 3))));
    ...
    % Modo Slip
    clear paso eta_x_ eta_xdd_ segundaderivadatheta segundaderivadax
    ...
    if tstart < tend
        ...
        F1_b_ = - masa * g * R * Mu .* cos(s1_(:, 3)) - masa * g * R * sin(s1_(:, 3)) + k * (s1_(:, 3) - theta_o);
        F1_ = - (F1_a_ + F1_b_);
        F2_= - masa * eta_xdd_ + masa * R * Mu .* cos(s1_(:, 3)) .* s1_(:, 4) .^ 2 - Mu * masa * g;
    ...
    % Vectores acumulativos
    theta_out = [theta_out; s1_(1:paso - 1, 3)];
    thetadot_out = [thetadot_out; s1_(1:paso - 1, 4)];
    xdot_out = [xdot_out; s1_(1:paso - 1, 2)];
    ydot_out = [ydot_out; - R * s1_(1:paso - 1, 4) .* sin(s1_(1:paso - 1, 3))];
    vpata_out = [vpata_out; s1_(1:paso - 1, 2) - R * s1_(1:paso - 1, 4) .* cos(s1_(1:paso - 1, 3))];
    ...
    eta_ydd_slip_out{end + 1} = 0;
    fuerzasy_slip_out{end + 1} = masa * g;
    fuerzayeta_slip_out{end + 1} = masa * eta_ydd_slip_out{end};
    normal_slip_out{end + 1} = masa * g + masa * (-R .* segundaderivadatheta(1:paso - 1) .* sin(s1_(1:paso - 1, 3)) - R .* s1_(1:paso - 1, 4) .^ 2 .* cos(s1_(1:paso - 1, 3)));
    friccion_slip_out{end + 1} = - uk * sign(s1_(1:paso - 1, 2) - R * s1_(1:paso - 1, 4) .* cos(s1_(1:paso - 1, 3))) .* (masa * g + masa * (-R .* segundaderivadatheta(1:paso - 1) .* sin(s1_(1:paso - 1, 3)) - R .* s1_(1:paso - 1, 4) .^ 2 .* cos(s1_(1:paso - 1, 3))));
    fuerzasx_slip_out{end + 1} = (masa .* segundaderivadax(1:paso - 1)) - uk * (masa * g + masa * (-R .* segundaderivadatheta(1:paso - 1) .* sin(s1_(1:paso - 1, 3)) - R .* s1_(1:paso - 1, 4) .^ 2 .* cos(s1_(1:paso - 1, 3))));
    ...
    clear paso eta_x_ eta_xdd  t_ s_ t1_ s1_ segundaderivadatheta segundaderivadax
